...
void main() {
    vec3 vPos = v_VertexPosition + vec3(0, Motion, 0);
    vec4 noise = node_noise_texture_2d(vPos * 2, 0, 14.4, 0, 1, 0);

    vec4 noisedPos = mix_linear(.1, vec4(vPos, 1), noise);

    vec4 middle = vec4(0,1,0,1) * math_compare(noisedPos.x, 0, .1) * 1.2;
    ...
    for(int i = 0; i < ActorAmount; i++) {
        ...
        float dis = distance(vec2(0, targetY), vE_ScaledPos);
        float compare = step(20, dis);
        float result = 1 - clamp(compare, 0, 1);

        yColor += Actor.Color * result;
    }

    float alpha = length(yColor) > .1 ? 0 : 1;
    middle *= alpha;
    vec3 result = vec3(middle) + vec3(yColor);
    Color = vec4(result, length(result));
}